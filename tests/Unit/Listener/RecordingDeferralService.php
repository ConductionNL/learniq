 ...
 * entry's shape AND replay it through the real background job. Asserting only
 * that `defer()` was called would prove the listener queued something — never
 * that the queued thing does the work the listener used to do inline. That gap
 * is exactly how a deferral conversion gets declared done while the behaviour
 * is gone.
 *
 * It does NOT reproduce the deferral service's buffering, dedupe or enqueue
 * behaviour; those belong to OpenRegister and are tested there. It records the
 * dedupe key so a test can assert the coalescing contract without depending on
 * OpenRegister's implementation of it.
 */
