...
use OCA\Learniq\Repair\RemoveRetiredCronJobs;
...
/**
 * @covers \OCA\Learniq\Repair\RemoveRetiredCronJobs
 *
 * @spec exclude No canonical spec covers the OCA\Learniq\Cron ->
 *  OCA\Learniq\BackgroundJob move; ADR-100 Decision 3 is an architecture record,
...
final class RemoveRetiredCronJobsTest extends TestCase {
	...
	public function testRemovesTheRegisteredRetiredClass(): void {
		...
		$this->assertContains('OCA\\Learniq\\Cron\\LtiAgsScorePollJob', $removed);
	...
	public function testAFailureOnOneClassDoesNotStopTheRest(): void {
		...
		$this->assertContains('OCA\\Learniq\\Cron\\LtiAgsScorePollJob', $attempted, 'every class must be attempted even when each one throws');
