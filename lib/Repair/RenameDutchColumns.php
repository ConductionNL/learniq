...
class RenameDutchColumns implements IRepairStep {
	 ...
	 *
	 * For rename-to-learniq: intentionally NOT renamed to 'learniq'. This step
	...
	private const COLUMN_MAP = [
		...
		'doorstroomtoets_level' => 'progression_test_level',
		'kerntaak_code' => 'core_task_code',
		'leerbedrijf_kvk_number' => 'training_company_kvk_number',
		'leerbedrijf_name' => 'training_company_name',
		'leerbedrijf_verification' => 'training_company_verification',
		'lesuur_minutes' => 'lesson_hour_minutes',
		'praktijkopleider_id' => 'practical_trainer_id',
		'schooladvies_adjusted_level' => 'school_advice_adjusted_level',
		'schooladvies_level' => 'school_advice_level',
		'shared_with_praktijkopleider_id' => 'shared_with_practical_trainer_id',
		'toelichting' => 'notes',
	];
	...
	public function getName(): string {
		return 'Move scholiq data from the Dutch columns to the English ones';
	}//end getName()

	/**
	 * Run the column migration across every scholiq shard table.
	 *
	...
	public function run(IOutput $output): void {
		...
		if ($tables === []) {
			$output->info('RenameDutchColumns: no scholiq shard tables on this install; nothing to do.');
			return;
		}
