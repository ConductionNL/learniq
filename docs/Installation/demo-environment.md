...
This page gets a working Learniq running on your own machine in two commands. You end with courses and learner records.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/learniq/development/learniq-compose.yaml
```
...
```bash
docker compose -f learniq-compose.yaml logs -f app-installer
```
...
==> installing learniq <version>
==> apps present: integriq learniq openregister thematiq
```
...
```bash
curl -s http://localhost:8612/status.php
```
...
| What | Where |
| --- | --- |
| **Learniq** | [http://localhost:8612/apps/learniq/](http://localhost:8612/apps/learniq/) |
| Admin interface | [http://localhost:8612](http://localhost:8612) — `admin` / `admin` |
...
| `learniq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing learniq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8612/apps/learniq/"
...
curl -s -u admin:admin "http://localhost:8612/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f learniq-compose.yaml down
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f learniq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
