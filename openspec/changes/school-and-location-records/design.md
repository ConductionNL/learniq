...
### Decision 4: the schema key/slug is `Vestiging`, not `Location`
`hydra-gate-cross-app-schema-slug` (gate-106) refused a schema slug `location`: it is already owned by `shillinq` on the shared OpenRegister, and slugs are global. Renaming to a compound name such as `SchoolLocation` was rejected: this register's `$ref`/schema-lookup mechanism resolves a literal reference string by lowercasing it against the schema's `slug`, with no camelCase-to-kebab conversion (the exact bug `slugify-ref-relation-resolver`, this round's nextcloud-vue fix, exists to close fleet-wide) — a compound PascalCase name would silently 404 the same way. `Vestiging` is a single word, free in `contracts/fleet-schema-slugs.json`, and is the literal Dutch term this schema already uses in `vestigingscode`. Only the internal schema key/slug/`$ref` target moved; every user-facing string (page title "Location", menu label "Locations", property titles) is unchanged.
