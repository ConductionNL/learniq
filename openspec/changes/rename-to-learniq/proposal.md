...
- The 117 literal `'scholiq'` register-slug/app-id string constants scattered across `lib/Controller/`, `lib/Lifecycle/`, `lib/BackgroundJob/`, `lib/Analytics/`, `lib/Engagement/`, `lib/Grading/`, `lib/CourseEvaluation/`, and sibling directories (measured via `git grep`; see design.md for the full boundary list).
